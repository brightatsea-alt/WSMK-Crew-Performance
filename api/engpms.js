...
module.exports = async (req, res) => {
  ...
  const user = { seafarer, rank, vessels: (vessels || []).slice(0, 12).map(v => ({ name: v, type: vType(v) })),
    ...
    recorded_events: { psc_deficiencies: (events.psc || []).slice(0, 25).map(x => cut(x, 220)), unscheduled_stoppage: (events.stoppage || []).slice(0, 12).map(x => cut(x, 240)), lti: (events.lti || []).slice(0, 10).map(x => cut(x, 220)) },
